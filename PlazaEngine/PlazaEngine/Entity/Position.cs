    ...
    public class Position
    {
        ...
        public void AddNewOpenOrder(Order openOrder)
        {
            ...
            else
            {
                ...
                {
                    if(SecurityName == openOrder.SecurityName)
                    {
                        _openOrders.Add(openOrder);
                    }
                }
        ...
        public void AddNewCloseOrder(Order closeOrder)
        {
            ...
            else
            {
                if (string.IsNullOrEmpty(SecurityName) == false
                    && SecurityName.EndsWith("TestPaper") == false)
                {
                    if(SecurityName == closeOrder.SecurityName)
                    {
                        _closeOrders.Add(closeOrder);
                    }
                }
        ...
        public bool OpenActiv
        {
            get
            {
                ...
                if (OpenOrders.Find(order => order.State == Order.OrderStateType.Activ 
                                             || order.State == Order.OrderStateType.Pending 
                                             || order.State == Order.OrderStateType.None
                                             || order.State == Order.OrderStateType.Partial) != null)
                {
                    return true;
                }
                return false;
        ...
        public bool CloseActiv
        {
            get
            {
                ...
                if (CloseOrders.Find(order => order.State == Order.OrderStateType.Activ 
                                              || order.State == Order.OrderStateType.Pending
                                              || order.State == Order.OrderStateType.None
                                              || order.State == Order.OrderStateType.Partial) != null
                    )
        ...
        public string SecurityName
        {
            get
            {
                ...
                {
                    return _openOrders[0].SecurityName;
                }
        ...
        public decimal WaitVolume
        {
            get
            {
                ...
                for (int i = 0; _openOrders != null && i < _openOrders.Count; i++)
                {
                    if (_openOrders[i].State == Order.OrderStateType.Activ ||
                        _openOrders[i].State == Order.OrderStateType.Partial)
                    {
        ...
        public decimal EntryPrice
        {
            get
            {
                ...
                if (volume == 0)
                {
                    return _openOrders[0].PriceReal;
                }
        ...
        public void SetOrder(Order newOrder)
        {
                ...
                for (int i = 0; i < _openOrders.Count; i++)
                {
                    if (_openOrders[i].NumberUser == newOrder.NumberUser)
                    {
                        if ((State == PositionStateType.Done || State == PositionStateType.OpeningFail) 
                            &&
                            ((_openOrders[i].State == Order.OrderStateType.Fail && newOrder.State == Order.OrderStateType.Fail) ||
                            (_openOrders[i].State == Order.OrderStateType.Cancel && newOrder.State == Order.OrderStateType.Cancel)))
                        {
                            return;
                        }
            ...
            if (openOrder != null)
            {
               if (openOrder.State != Order.OrderStateType.Done 
                    || openOrder.Volume != openOrder.VolumeExecute)    //AVP 
                {
                    openOrder.state = newOrder.State;     //AVP 
                }
                openOrder.SetNumberMarket(newOrder.NumberMarket);
                ...
                else if (newOrder.State == Order.OrderStateType.Fail 
                    && newOrder.VolumeExecute == 0 
                    && OpenVolume == 0)
                {
                    State = PositionStateType.OpeningFail;
                }
                else if (newOrder.State == Order.OrderStateType.Cancel
                    && newOrder.VolumeExecute == 0 
                    && OpenVolume == 0)
                {
                    State = PositionStateType.OpeningFail;
                ...
                else if (newOrder.State == Order.OrderStateType.Done 
                    && OpenVolume == 0 
                    && CloseOrders != null 
                    && CloseOrders.Count > 0)
                {
            ...
            if (CloseOrders != null)
            {
                for (int i = 0; i < CloseOrders.Count; i++)
                {
                    if (CloseOrders[i].NumberUser == newOrder.NumberUser)
                    {
                        if (
                                (
                                (CloseOrders[i].State == Order.OrderStateType.Fail && newOrder.State == Order.OrderStateType.Fail) 
                                ||
                                (CloseOrders[i].State == Order.OrderStateType.Cancel && newOrder.State == Order.OrderStateType.Cancel)
                                )
                                &&
            ...
            if (closeOrder != null)
            {
                ...
                closeOrder.SetNumberMarket(newOrder.NumberMarket);
                ...
                if (closeOrder.State == Order.OrderStateType.Done && OpenVolume == 0)
                {
                    //AlertMessageManager.ThrowAlert(null, "Done", "");
                    State = PositionStateType.Done;
                }
                else if (closeOrder.State == Order.OrderStateType.Fail && !CloseActiv && OpenVolume != 0)
                {
                    //AlertMessageManager.ThrowAlert(null, "Fail", "");
                    State = PositionStateType.ClosingFail;
                }
                else if (closeOrder.State == Order.OrderStateType.Cancel && !CloseActiv && OpenVolume != 0)
                {
                ...
                else if (closeOrder.State == Order.OrderStateType.Done && OpenVolume < 0)
                {
            ...
            if (_openOrders != null)
            {
                ...
                for (int i = 0; i < _openOrders.Count; i++)
                {
                    if ((_openOrders[i].NumberMarket == trade.NumberOrderParent
                        ...
                        && _openOrders[i].SecurityName== trade.SecurityName)
                    {
            ...
            if (CloseOrders != null)
            {
                for (int i = 0; i < CloseOrders.Count; i++)
                {
                    if ((CloseOrders[i].NumberMarket == trade.NumberOrderParent 
                        ...
                        && CloseOrders[i].SecurityName== trade.SecurityName)
                    {
                        trade.NumberPosition = Number.ToString();
        ...
        public DateTime TimeOpen
        {
            get
            {
                ...
                for (int i = 0; i < OpenOrders.Count; i++)
                {
                    if (OpenOrders[i].MyTrades?.Count() > 0 &&
